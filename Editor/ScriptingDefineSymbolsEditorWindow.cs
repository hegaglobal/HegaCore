...
public class ScriptingDefineSymbolsEditorWindow : EditorWindow
{
    ...
    private static readonly GUIContent CONTENT_BUTTON_APPLY = new GUIContent("Apply",
        "Apply all Scripting Define Symbols changes. This will made unity recompile if your current build target group is active.");

    [MenuItem("Tools/Scripting Define Symbols Editor", false)]
    ...
    private void OnGUI()
    {
        using (var horizontalScope = new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
        {
            ...
            {
                inspectorBuildTargetGroup = (BuildTargetGroup) EditorGUILayout.EnumPopup(GUIContent.none,
                    buildTargetGroup, EditorStyles.toolbarDropDown, GUILayout.Width(100.0f));
                if (changeScope.changed && buildTargetGroup != inspectorBuildTargetGroup)
                {
                    if (isDirty)
                    {
                        if (EditorUtility.DisplayDialog("Warning",
                            "All changes on current active build target will be revert. Do you want to apply change?",
                            "Yes", "No"))
                        {
                            ApplyChangesScriptingDefineSymbols();
                        }
        ...
        {
            EditorGUILayout.HelpBox("Scripting Define Symbols is disabled on this build target group.",
